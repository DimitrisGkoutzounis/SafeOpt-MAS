...
class Agent(object):
    ...
    def __init__(self,agent_id):
        ...
        #set bounds max 2
        self.bounds =[-0.8,0.8],[-0.0,0.0]
        self.init_action = np.array([0.0,0.0])

        self.action = {f'Agent{self.id}': self.init_action} #initialize empty action for each agent
        self.local_reward = np.asarray([[0]]) #initialize empty reward for each agent

        self.opt = self.initialize_optimization()
    ...
    def initialize_optimization(self):
        ...
        self.opt = SafeOpt(self.gp, parameter_set, fmin=-3.5,threshold=0.4 ,beta=3.5)
